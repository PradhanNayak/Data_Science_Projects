...
/*-- Type your code below: total 9 columns are present in table movie.*/
...
/* TO CHECK NO OF MOVIES RELEASED EACH YEAR */
SELECT year AS Year,
...
/* TO CHECK MOVIES RELEASED MONTHWISE*/
...
/*-- Type your code below:*/
SELECT COUNT(one_genre.movie_id) AS one_genre_movies
FROM(SELECT movie_id, COUNT(*) FROM genre GROUP BY movie_id HAVING COUNT(*) = 1) one_genre;
...
-- Drama has the highest count with 16.
...
-- The Brighton Miracle tops the average rating in the genre Drama.
...
GROUP BY median_rating;
-- 361 movies were given a Median rating 8 Of the movies released between 1 April 2018 and 1 April 2019.
...
THEN 'yes'
ELSE 'no'
END AS Germany_has_more_votes_than_Italy;
...
ON m.id=rm.movie_id INNER JOIN names n
ON rm.name_id=n.id
...
-- Star cinema nad Twentieth century fox are the top two production houses.
...
total_votes,
COUNT(m.id) AS movie_count,
ROW_NUMBER() OVER(ORDER BY avg_rating>8 DESC) AS actress_rank
FROM movie m INNER JOIN genre g
ON m.id=g.movie_id INNER JOIN ratings r
ON m.id=r.movie_id INNER JOIN role_mapping rm
ON m.id=rm.movie_id INNER JOIN names n
...
GROUP BY name
LIMIT 3;
-- Kim hunter, kathleen Byron, Yolonda ross are the top 3 actress.
...
-- Andrew jones and A.L.Vijay top the directors based on number of movies.
